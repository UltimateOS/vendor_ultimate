...
func init() {
	android.RegisterModuleType("ultimate_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ultimateExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ultimateExpandVariables(ctx, dep_file)
	...
	cmd := ultimateExpandVariables(ctx, String(g.properties.Cmd))
